...
class DeliverySuperAdmin extends BaseController
{
    ...
    public function fetch_invoice()
    {
        ...
        $data_table->setTable($delivery_super_model->fetch_all_delivery())
            ...
            ->setOutput([
                ...
                "recipient_name", "recipient_phone", "recipient_address",
                "recipient_area", "recipient_instructions", "delivery_status",
                $delivery_super_model->edit_button()
            ]);
        ...
        if ($this->request->getVar('action')) {
            ...
            if (!$error) {
                ...
                if ($this->request->getVar('action') == 'Add') {
                    ...
                    $err_message = '<div class="alert alert-success">Invoice Created Sucessfully!</div>';
                }
                if ($this->request->getVar('action') == 'Edit') {
                    ...
                    $err_message = '<div class="alert alert-success">Invoice Updated Sucessfully!</div>';
                }
            }
            $output = array(
    ...
    public function fetch_invoice_by_id()
    {
        if ($this->request->getVar('id')) {
            $db = db_connect();
            $delivery_super_model = new SuperFunctionsModel($db);
            $invoice_data = $delivery_super_model->get_invoice_by_id($this->request->getVar('id'));
            return json_encode($invoice_data);
        }
    }

    public function del_invoice_by_id()
    {
        if ($this->request->getVar('id')) {
            ...
            return '<div class="alert alert-danger">Invoice Deleted. I hope you know what you are doing!</div>';
        }
    }

    //traditional-non-ajax-methods
